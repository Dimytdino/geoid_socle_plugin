...
| S-15 | Mettre en œuvre le template projet dédié (ADR-003, option B) : créer le dépôt `geoid-projet-template` (org, privé, *template*) ; y placer le résiduel + `.claude/settings.json` avec `extraKnownMarketplaces`/`enabledPlugins` (`autoUpdate`) ; définir la synchro socle → template ; mettre à jour DEMARRER/README/bootstrap (l'install `/plugin` manuelle quitte le chemin nominal) ; retirer au dépôt socle son rôle de template | Haute | mainteneur / architecte | À faire (2026-07-29) — décision ADR-003 actée ; création du dépôt = action org à confirmer | — |
